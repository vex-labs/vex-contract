...
async fn test_error_at_future() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Cancel match
    result = cancel_match(admin.clone(), vex_contract.id(), "RUBY-Nexus-17/08/2024").await?;
    ...
    assert!(
        result.is_failure(),
        "Admin managed to end betting after match was cancelled"
    );
    ...
    assert!(
        result.is_failure(),
        "Admin managed to finish match after match was cancelled"
    );
    ...
    let bet = vex_contract
        .view("get_bet")
        .args_json(serde_json::json!({"bettor": alice.id(), "bet_id": U64(3)}))
        .await;
    assert!(
        bet.is_err(),
    ...
    result = claim(alice.clone(), vex_contract.id(), U64(1)).await?;
    ...
    result = claim(bob.clone(), vex_contract.id(), U64(2)).await?;
